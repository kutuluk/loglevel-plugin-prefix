(function (root, factory) {
  ...
  var apply = function (logger, config) {
    ...
    function methodFactory(methodName, logLevel, loggerName) {
      ...
      var hasLineNumber = options.template.indexOf('%m') !== -1;
      return function () {
        var content = '';
	let trace_ = new Error().stack;
        let traceLog = trace_.split('at');
        let findLoggerIdx = traceLog.findIndex(function(__log){ return (__log.indexOf('g.')!==-1 || __log.indexOf('Logger')!==-1) });
        let lineNumberURI = traceLog[findLoggerIdx+1];
	    ...
	    if (hasLineNumber) {
		 ...
		 args[0] += lineNumberURI;
	    }
          } else {
	     if (hasLineNumber) args.push(lineNumberURI);
